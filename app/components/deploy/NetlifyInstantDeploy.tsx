...
interface NetlifyInstantDeployProps {
...
export function NetlifyInstantDeploy({
  variant = 'primary',
  size = 'md',
  showIcon = true,
  children,
  ...
  onSuccess,
  onError
}: NetlifyInstantDeployProps) {
  const [isDeploying, setIsDeploying] = useState(false);
  const [step, setStep] = useState<'building' | 'creating' | 'deploying' | 'complete'>('building');
  ...
  const handleInstantDeploy = async () => {
    ...
    try {
      ...
      // Step 2: Create site and deploy
      setStep('creating');
      toast.info('Creating Netlify site...');
      ...
      // Create site using public API (no authentication required for demo)
      const siteName = `digitalisasi-ai-instant-${Date.now()}`;
  ...
  return (
    ...
    >
      ...
      {isDeploying ? (
        ...
        children || 'Instant Deploy to Netlify'
      )}
    </button>
  );
}
...
// Quick instant deploy component
export function NetlifyQuickInstantDeploy() {
  return (
    <NetlifyInstantDeploy
      variant="primary"
      size="md"
    >
...
// Advanced instant deploy component
export function NetlifyAdvancedInstantDeploy({ siteName }: { siteName?: string }) {
  return (
    <NetlifyInstantDeploy
      variant="secondary"
      size="lg"
